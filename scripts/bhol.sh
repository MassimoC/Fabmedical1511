...
if [[ -z "${MCW_GITHUB_URL}" ]]; then
    MCW_GITHUB_URL=https://$MCW_GITHUB_TOKEN@github.com/$MCW_GITHUB_USERNAME/Fabmedical1511.git
fi

git config --global user.email "$MCW_GITHUB_EMAIL"
git config --global user.name "$MCW_GITHUB_USERNAME"

cp -R ~/MCW-Cloud-native-applications/Hands-on\ lab/lab-files/developer ~/Fabmedical1511
cd ~/Fabmedical1511
git init
...
cd ~/Fabmedical1511
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical1511/.github/workflows/content-init.yml
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical1511/.github/workflows/content-api.yml
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical1511/.github/workflows/content-web.yml
